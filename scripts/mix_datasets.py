...
from nlp.utils import read_json_or_jsonl, write_jsonl, shuffle_and_split


def main():
    parser = argparse.ArgumentParser(description="将 identity.jsonl 与 history_qa.jsonl 混合、打乱并划分 train/val")
    parser.add_argument("--identity", type=str, required=True, help="identity.jsonl 路径")
    parser.add_argument("--history", type=str, required=True, help="history_qa.jsonl 路径")
    ...
    identity_items: List[Dict[str, Any]] = read_json_or_jsonl(args.identity)
    history_items: List[Dict[str, Any]] = read_json_or_jsonl(args.history)
